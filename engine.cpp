...
std::default_random_engine  rnd_d(std::chrono::steady_clock::now().time_since_epoch().count());
...
bool					bLandPresetMode;

// (Toxicity) this is needed for Ascension
MapSaveFile				MapSave;
int						nFlagPaintSelected = 0;
int						nFlagPaintValue = 0;
...
OBJ3D					*objAodBlue = 0,
						...
						*objWarriorTrainCyan = 0,
						*objWarriorTrainMagenta = 0,
						*objWarriorTrainBlack = 0,
						*objWarriorTrainOrange = 0,
						*objTempleBlue = 0,
						*objTempleRed = 0,
						*objTempleYellow = 0,
						*objTempleGreen = 0,
						...
						*objBallonHutGreen = 0,
						*objBallonHutCyan = 0,
						*objBallonHutMagenta = 0,
						*objBallonHutBlack = 0,
						*objBallonHutOrange = 0,
						*objPrison = 0,
						*objRSPillar = 0,
						*objRSPillarBlue = 0,
						*objRSPillarRed = 0,
...
void GetMatrixFromLand(WORD (&matrix)[128][128], WORD (&land)[128*128])
{
	...
	for (int x = 0; x < 128; x++)
	{
		for (int z = 0; z < 128; z++)
		{
			...
			h = land[z * GROUND_Z_SIZE + x];
			matrix[x][z] = h;
		}
	}
}

void MatrixToLand(WORD(&matrix)[128][128], WORD(&land)[128 * 128])
{
	for (int x = 0; x < 128; x++)
	{
		for (int z = 0; z < 128; z++)
		{
			...
			while (x >= GROUND_X_SIZE) x -= GROUND_X_SIZE;
			while (z >= GROUND_Z_SIZE) z -= GROUND_Z_SIZE;
			land[z * GROUND_Z_SIZE + x] = matrix[x][z];
		}
	}
}
...
void InitMapSaveElements()
{
	for (int i = 0; i < MAP_SIZE; i++)
	{
		MapSave.Elements[i].TextureAlt = -1;
	}
}
...
long EngineDrawFlag(int x, int z)
{
	while (x < 0) x += GROUND_X_SIZE;
	while (z < 0) z += GROUND_Z_SIZE;
	while (x >= GROUND_X_SIZE) x -= GROUND_X_SIZE;
	...
	case FPI_TEXTURE_ALT:
		sprintf(str, "X=%d Z=%d TexAlt=%d", x, z, MapSave.Elements[z * GROUND_Z_SIZE + x].TextureAlt);
		break;
	default:
		sprintf(str, "X=%d Z=%d", x, z);
		break;
	}
...
_skip:
			...
			if (Preset.Land.size() && bLandPresetMode && !fFlagPaint)
			{
				int _x, _z;
				int i = 0;
				int ax, az;
				...
				while (az <= Preset.LandSize)
				{
					...
					while (ax <= Preset.LandSize)
					{
						...
						{
							_x = (int)ex - (Preset.LandSize / 2);
							_z = (int)ez - (Preset.LandSize / 2);
						}

						_x = _x + ax;
						_z = _z + az;

						while (_x < 0) _x += GROUND_X_SIZE;
						while (_z < 0) _z += GROUND_Z_SIZE;
						while (_x >= GROUND_X_SIZE) _x -= GROUND_X_SIZE;
						while (_z >= GROUND_Z_SIZE) _z -= GROUND_Z_SIZE;
	
						wLandPreset[_z * GROUND_Z_SIZE + _x] = Preset.Land[i].h;
						ax++;
						i++;
					}
					az++;
			...
			EngineDrawFlag((int)ex - (GroundEditBrushSize / 2), (int)ez - (GroundEditBrushSize / 2));

			if (fCaptured && !bLandPresetMode)
			{
				if (fFlagPaint && nFlagPaintSelected)
				{
					int _x, _z;
					int ax, az;
					...
					while (az <= GroundEditBrushSize)
					{
						...
						{
							_x = (int)ex - (GroundEditBrushSize / 2);
							_z = (int)ez - (GroundEditBrushSize / 2);
							...
							if (MouseButton == MouseButtonRight)
								MapSave.Elements[_z * GROUND_Z_SIZE + _x].TextureAlt = -1;
							else
								MapSave.Elements[_z * GROUND_Z_SIZE + _x].TextureAlt = nFlagPaintValue;
							ax++;
						}
						az++;
					}
...
long EngineDrawObjects()
{
	...
	if(Things)
	{
		...
		do
		{
			...
			if(thing->flags & TF_DRAW)
			{
				...
				case T_PERSON:
					...
					case M_PERSON_BRAVE:
						...
						case OWNER_CYAN:	lpD3DDevice->SetTexture(0, txBraveCyan->lpDDSTexture);   break;
						case OWNER_MAGENTA:		lpD3DDevice->SetTexture(0, txBraveMagenta->lpDDSTexture);    break;
						case OWNER_BLACK:	lpD3DDevice->SetTexture(0, txBraveBlack->lpDDSTexture); break;
						case OWNER_ORANGE:	lpD3DDevice->SetTexture(0, txBraveOrange->lpDDSTexture);  break;
						default: goto unknow;
						}
						break;
					case M_PERSON_WARRIOR:
						...
						case OWNER_CYAN:	lpD3DDevice->SetTexture(0, txWarriorCyan->lpDDSTexture);   break;
						case OWNER_MAGENTA:		lpD3DDevice->SetTexture(0, txWarriorMagenta->lpDDSTexture);    break;
						case OWNER_BLACK:	lpD3DDevice->SetTexture(0, txWarriorBlack->lpDDSTexture); break;
						case OWNER_ORANGE:	lpD3DDevice->SetTexture(0, txWarriorOrange->lpDDSTexture);  break;
						default: goto unknow;
						}
						break;
					case M_PERSON_PRIEST:
						...
						case OWNER_CYAN:	lpD3DDevice->SetTexture(0, txPriestCyan->lpDDSTexture);   break;
						case OWNER_MAGENTA:		lpD3DDevice->SetTexture(0, txPriestMagenta->lpDDSTexture);    break;
						case OWNER_BLACK:	lpD3DDevice->SetTexture(0, txPriestBlack->lpDDSTexture); break;
						case OWNER_ORANGE:	lpD3DDevice->SetTexture(0, txPriestOrange->lpDDSTexture);  break;
						default: goto unknow;
						}
						break;
					case M_PERSON_SPY:
						...
						case OWNER_CYAN:	lpD3DDevice->SetTexture(0, txSpyCyan->lpDDSTexture);   break;
						case OWNER_MAGENTA:		lpD3DDevice->SetTexture(0, txSpyMagenta->lpDDSTexture);    break;
						case OWNER_BLACK:	lpD3DDevice->SetTexture(0, txSpyBlack->lpDDSTexture); break;
						case OWNER_ORANGE:	lpD3DDevice->SetTexture(0, txSpyOrange->lpDDSTexture);  break;
						default: goto unknow;
						}
						break;
					case M_PERSON_FIREWARRIOR:
						...
						case OWNER_CYAN:	lpD3DDevice->SetTexture(0, txFwCyan->lpDDSTexture);   break;
						case OWNER_MAGENTA:		lpD3DDevice->SetTexture(0, txFwMagenta->lpDDSTexture);    break;
						case OWNER_BLACK:	lpD3DDevice->SetTexture(0, txFwBlack->lpDDSTexture); break;
						case OWNER_ORANGE:	lpD3DDevice->SetTexture(0, txFwOrange->lpDDSTexture);  break;
						default: goto unknow;
						}
						break;
            ...
            case OWNER_CYAN:	lpD3DDevice->SetTexture(0, txShamanCyan->lpDDSTexture);   break;
						case OWNER_MAGENTA:		lpD3DDevice->SetTexture(0, txShamanMagenta->lpDDSTexture);    break;
						case OWNER_BLACK:	lpD3DDevice->SetTexture(0, txShamanBlack->lpDDSTexture); break;
						case OWNER_ORANGE:	lpD3DDevice->SetTexture(0, txShamanOrange->lpDDSTexture);  break;
						default: goto unknow;
						}
						break;
				...
				case T_BUILDING:
					...
					case M_BUILDING_HUT1:
						...
						case OWNER_CYAN:    EngineDrawObj3D(objHut1Cyan, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						case OWNER_MAGENTA: EngineDrawObj3D(objHut1Magenta, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						case OWNER_BLACK:    EngineDrawObj3D(objHut1Black, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						case OWNER_ORANGE: EngineDrawObj3D(objHut1Orange, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						default: goto unknow;
						}
						break;
					case M_BUILDING_HUT2:
						...
						case OWNER_CYAN:    EngineDrawObj3D(objHut2Cyan, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						case OWNER_MAGENTA: EngineDrawObj3D(objHut2Magenta, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						case OWNER_BLACK:    EngineDrawObj3D(objHut2Black, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						case OWNER_ORANGE: EngineDrawObj3D(objHut2Orange, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						default: goto unknow;
						}
						break;
					case M_BUILDING_HUT3:
						...
						case OWNER_CYAN:    EngineDrawObj3D(objHut3Cyan, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						case OWNER_MAGENTA: EngineDrawObj3D(objHut3Magenta, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						case OWNER_BLACK:    EngineDrawObj3D(objHut3Black, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						case OWNER_ORANGE: EngineDrawObj3D(objHut3Orange, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						default: goto unknow;
						}
						break;
					case M_BUILDING_TOWER:
						...
						case OWNER_CYAN:   EngineDrawObj3D(objTowerCyan, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);   break;
						case OWNER_MAGENTA:    EngineDrawObj3D(objTowerMagenta, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);    break;
						case OWNER_BLACK: EngineDrawObj3D(objTowerBlack, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle); break;
						case OWNER_ORANGE:  EngineDrawObj3D(objTowerOrange, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						default: goto unknow;
						}
						break;
					case M_BUILDING_SPY_TRAIN:
						...
						case OWNER_CYAN:   EngineDrawObj3D(objSpyTrainCyan, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);   break;
						case OWNER_MAGENTA:    EngineDrawObj3D(objSpyTrainMagenta, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);    break;
						case OWNER_BLACK: EngineDrawObj3D(objSpyTrainBlack, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle); break;
						case OWNER_ORANGE:  EngineDrawObj3D(objSpyTrainOrange, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						default: goto unknow;
						}
						break;
					case M_BUILDING_WARRIOR_TRAIN:
						...
						case OWNER_CYAN:   EngineDrawObj3D(objWarriorTrainCyan, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);   break;
						case OWNER_MAGENTA:    EngineDrawObj3D(objWarriorTrainMagenta, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);    break;
						case OWNER_BLACK: EngineDrawObj3D(objWarriorTrainBlack, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle); break;
						case OWNER_ORANGE:  EngineDrawObj3D(objWarriorTrainOrange, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						default: goto unknow;
						}
						break;
					case M_BUILDING_TEMPLE:
						...
						case OWNER_CYAN:   EngineDrawObj3D(objTempleCyan, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);   break;
						case OWNER_MAGENTA:    EngineDrawObj3D(objTempleMagenta, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);    break;
						case OWNER_BLACK: EngineDrawObj3D(objTempleBlack, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle); break;
						case OWNER_ORANGE:  EngineDrawObj3D(objTempleOrange, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						default: goto unknow;
						}
						break;
					case M_BUILDING_FIREWARRIOR_TRAIN:
						...
						case OWNER_CYAN:   EngineDrawObj3D(objFwTrainCyan, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);   break;
						case OWNER_MAGENTA:    EngineDrawObj3D(objFwTrainMagenta, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);    break;
						case OWNER_BLACK: EngineDrawObj3D(objFwTrainBlack, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle); break;
						case OWNER_ORANGE:  EngineDrawObj3D(objFwTrainOrange, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						default: goto unknow;
						}
						break;
					...
					case M_BUILDING_BOAT_HUT_2:
						...
						case OWNER_CYAN:   EngineDrawObj3D(objBoatHutCyan, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);   break;
						case OWNER_MAGENTA:    EngineDrawObj3D(objBoatHutMagenta, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);    break;
						case OWNER_BLACK: EngineDrawObj3D(objBoatHutBlack, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle); break;
						case OWNER_ORANGE:  EngineDrawObj3D(objBoatHutOrange, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						default: goto unknow;
						}
						break;
					...
					case M_BUILDING_AIRSHIP_HUT_2:
						...
						case OWNER_CYAN:   EngineDrawObj3D(objBallonHutCyan, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);   break;
						case OWNER_MAGENTA:    EngineDrawObj3D(objBallonHutMagenta, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);    break;
						case OWNER_BLACK: EngineDrawObj3D(objBallonHutBlack, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle); break;
						case OWNER_ORANGE:  EngineDrawObj3D(objBallonHutOrange, thing->ex, thing->ey, thing->ez, thing->Thing.Building.Angle);  break;
						default: goto unknow;
						}
						break;
...
skip:;
	...
	if(fEngineEditMarkers || (dwEngineFlags & EF_SHOW_MARKERS))
	{
		...
		else if (fCaptured && !fMoving && fEngineEditMarkers && (MarkerSelected == -1) && !fNewMarkerAdded && bKeys[VK_CONTROL])
		{
			D3DVECTOR r0, r1;
			EngineGetPick(&r0, &r1);
...
long EngineLoadLevel(char *filename)
{
	...
	dwRW = 0;
	ReadFile(h, &MapSave, sizeof(MapSaveFile), &dwRW, 0);
	if (dwRW != sizeof(MapSaveFile))
	{
		CloseHandle(h);
...
long EngineSaveLevelV3(char *filename)
{
	...
	if(t) do
	{
		...
		{
			t->Thing.PosX = (((int)t->x * 2) + 1) << 8;
			t->Thing.PosZ = (((int)t->z * 2) + 1) << 8;
		}
...
long EngineSaveLevel(char *filename)
{
	...
	dwRW = 0;
	WriteFile(h, &MapSave, sizeof(MapSaveFile), &dwRW, 0);
	if (dwRW != sizeof(MapSaveFile))
	{
		CloseHandle(h);
...
long EngineUpdateView()
{
	...
	{
		DWORD color = 0x3CC700; //0xC70000;
		WORD w = EngineGetGroundHeightPreset(x, z);

		if (w <= 0)
		{
			color = 0;
			w = EngineGetGroundHeight(x, z);
		}
		auto _x = x;
		auto _z = z;
		...
		while (_x >= GROUND_X_SIZE) _x -= GROUND_X_SIZE;
		while (_z >= GROUND_Z_SIZE) _z -= GROUND_Z_SIZE;
		auto texAlt = MapSave.Elements[_z * GROUND_Z_SIZE + _x].TextureAlt;
		if (texAlt != -1)
			color = 0xC70000;
		...
		if (color)
			GroundHeight[ax][az].color = color;
		else
...
WORD EngineGetGroundHeightPreset(int x, int z)
{
	while (x < 0) x += GROUND_X_SIZE;
	while (z < 0) z += GROUND_Z_SIZE;
	while (x >= GROUND_X_SIZE) x -= GROUND_X_SIZE;
	while (z >= GROUND_Z_SIZE) z -= GROUND_Z_SIZE;
	return wLandPreset[z * GROUND_Z_SIZE + x];
}

void EngineSetGroundHeightPreset(int x, int z, WORD h)
{
	while (x < 0) x += GROUND_X_SIZE;
	while (z < 0) z += GROUND_Z_SIZE;
	while (x >= GROUND_X_SIZE) x -= GROUND_X_SIZE;
	while (z >= GROUND_Z_SIZE) z -= GROUND_Z_SIZE;
	wLandPreset[z * GROUND_Z_SIZE + x] = h;
}
...
up_skip:
	...
	// ctrl + c (copy land as preset)
	if (bKeys[VK_CONTROL] && bKeys[0x43])
	{
		if (GroundEditBrushSize)
		{
			printf("Copied land piece as preset\n");
			D3DVECTOR r0, r1;
			EngineGetPick(&r0, &r1);
	...
	if (bKeys[VK_CONTROL] && bKeys[0x56])
	{
		if (Preset.Land.size())
		{
			printf("Entered Land Preset mode\n");
			bLandPresetMode = true;
		}
		else
			printf("There's no preset stored. Couldn't enter Land Preset mode.\n");
		bKeys[0x56] = 0;
	}
	...
	if (bKeys[0x52])
	{
		...
		else // rotate normal land (not preset)
		{
			WORD matrix[128][128];
			GetMatrixFromLand(matrix, wEngineGround);
			Matrix_Rotate90(matrix);
			MatrixToLand(matrix, wEngineGround);
	...
	if (bKeys[VK_OEM_PLUS])
	{
		...
		if (bKeys[VK_SHIFT])
		{
			...
			std::rotate
			(
				std::begin(wEngineGround),
				std::begin(wEngineGround) + std::distance(std::begin(wEngineGround), std::end(wEngineGround)) - 1,
				std::end(wEngineGround)
			);
	...
	if (bKeys[VK_OEM_MINUS])
	{
		...
		if (bKeys[VK_SHIFT])
		{
			...
			std::rotate(std::begin(wEngineGround), std::begin(wEngineGround) + 1, std::end(wEngineGround));
			EngineUpdateView();
			EngineUpdateMiniMap();
			bKeys[VK_OEM_MINUS] = 0;
		}
		else
		{
	...
	if(Things && !(dwEngineFlags & EF_HIDE_OBJS_ON_MINIMAP))
	{
		...
		do
		{
			...
			if(IsNotNeutral(t))
			{
				...
				{
				case OWNER_BLUE:		c = 0xFF2200; break;
				case OWNER_RED:			c = 0x0000FF; break;
				case OWNER_YELLOW:		c = 0x00FFFF; break;
				case OWNER_GREEN:		c = 0x00FF00; break;
				case OWNER_CYAN:		c = 0xFFFF00; break;
				case OWNER_MAGENTA:		c = 0xFF00FF; break;
				case OWNER_BLACK:		c = 0x2E2E2E; break;
				case OWNER_ORANGE:		c = 0x03A9FC; break;
				default: c = 0xC0C0C0;
				}
			}
			else
...
bool EngineLoadObjects()
{
	...
	txBraveCyan = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_BRAVE_CYAN, 0);
	txBraveMagenta = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_BRAVE_MAGENTA, 0);
	txBraveBlack = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_BRAVE_BLACK, 0);
	txBraveOrange = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_BRAVE_ORANGE, 0);
	...
	txSpyCyan = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_SPY_CYAN, 0);
	txSpyMagenta = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_SPY_MAGENTA, 0);
	txSpyBlack = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_SPY_BLACK, 0);
	txSpyOrange = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_SPY_ORANGE, 0);
	...
	txWarriorCyan = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_WARRIOR_CYAN, 0);
	txWarriorMagenta = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_WARRIOR_MAGENTA, 0);
	txWarriorBlack = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_WARRIOR_BLACK, 0);
	txWarriorOrange = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_WARRIOR_ORANGE, 0);
	...
	txPriestCyan = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_PRIEST_CYAN, 0);
	txPriestMagenta = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_PRIEST_MAGENTA, 0);
	txPriestBlack = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_PRIEST_BLACK, 0);
	txPriestOrange = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_PRIEST_ORANGE, 0);
	...
	txFwCyan = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_FW_CYAN, 0);
	txFwMagenta = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_FW_MAGENTA, 0);
	txFwBlack = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_FW_BLACK, 0);
	txFwOrange = EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_FW_ORANGE, 0);
	...
	txShamanCyan	= EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_SHAMAN_CYAN, 0);
	txShamanMagenta		= EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_SHAMAN_MAGENTA, 0);
	txShamanBlack	= EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_SHAMAN_BLACK, 0);
	txShamanOrange	= EngineCreateTexture(PERSON_TEXTURE_SIZE, PERSON_TEXTURE_SIZE, SZ_TX_SHAMAN_ORANGE, 0);
	...
	objHut1Cyan				= _3dsLoadObj(SZ_OBJ_HUT1_CYAN);
	objHut2Cyan				= _3dsLoadObj(SZ_OBJ_HUT2_CYAN);
	objHut3Cyan				= _3dsLoadObj(SZ_OBJ_HUT3_CYAN);
	objHut1Magenta			= _3dsLoadObj(SZ_OBJ_HUT1_MAGENTA);
	objHut2Magenta			= _3dsLoadObj(SZ_OBJ_HUT2_MAGENTA);
	objHut3Magenta			= _3dsLoadObj(SZ_OBJ_HUT3_MAGENTA);
	objHut1Black			= _3dsLoadObj(SZ_OBJ_HUT1_BLACK);
	objHut2Black			= _3dsLoadObj(SZ_OBJ_HUT2_BLACK);
	objHut3Black			= _3dsLoadObj(SZ_OBJ_HUT3_BLACK);
	objHut1Orange			= _3dsLoadObj(SZ_OBJ_HUT1_ORANGE);
	objHut2Orange			= _3dsLoadObj(SZ_OBJ_HUT2_ORANGE);
	objHut3Orange			= _3dsLoadObj(SZ_OBJ_HUT3_ORANGE);
	...
	objTowerCyan			= _3dsLoadObj(SZ_OBJ_TOWER_CYAN);
	objTowerMagenta			= _3dsLoadObj(SZ_OBJ_TOWER_MAGENTA);
	objTowerBlack			= _3dsLoadObj(SZ_OBJ_TOWER_BLACK);
	objTowerOrange			= _3dsLoadObj(SZ_OBJ_TOWER_ORANGE);
	...
	objSpyTrainCyan			= _3dsLoadObj(SZ_OBJ_SPY_TRAIN_CYAN);
	objSpyTrainMagenta		= _3dsLoadObj(SZ_OBJ_SPY_TRAIN_MAGENTA);
	objSpyTrainBlack		= _3dsLoadObj(SZ_OBJ_SPY_TRAIN_BLACK);
	objSpyTrainOrange		= _3dsLoadObj(SZ_OBJ_SPY_TRAIN_ORANGE);
	...
	objWarriorTrainCyan		= _3dsLoadObj(SZ_OBJ_WARRIOR_TRAIN_CYAN);
	objWarriorTrainMagenta	= _3dsLoadObj(SZ_OBJ_WARRIOR_TRAIN_MAGENTA);
	objWarriorTrainBlack	= _3dsLoadObj(SZ_OBJ_WARRIOR_TRAIN_BLACK);
	objWarriorTrainOrange	= _3dsLoadObj(SZ_OBJ_WARRIOR_TRAIN_ORANGE);
	...
	objTempleCyan			= _3dsLoadObj(SZ_OBJ_TEMPLE_CYAN);
	objTempleMagenta		= _3dsLoadObj(SZ_OBJ_TEMPLE_MAGENTA);
	objTempleBlack			= _3dsLoadObj(SZ_OBJ_TEMPLE_BLACK);
	objTempleOrange			= _3dsLoadObj(SZ_OBJ_TEMPLE_ORANGE);
	...
	objFwTrainCyan			= _3dsLoadObj(SZ_OBJ_FW_TRAIN_CYAN);
	objFwTrainMagenta		= _3dsLoadObj(SZ_OBJ_FW_TRAIN_MAGENTA);
	objFwTrainBlack			= _3dsLoadObj(SZ_OBJ_FW_TRAIN_BLACK);
	objFwTrainOrange		= _3dsLoadObj(SZ_OBJ_FW_TRAIN_ORANGE);
	...
	objBoatHutCyan			= _3dsLoadObj(SZ_OBJ_BOAT_HUT_CYAN);
	objBoatHutMagenta		= _3dsLoadObj(SZ_OBJ_BOAT_HUT_MAGENTA);
	objBoatHutBlack			= _3dsLoadObj(SZ_OBJ_BOAT_HUT_BLACK);
	objBoatHutOrange		= _3dsLoadObj(SZ_OBJ_BOAT_HUT_ORANGE);
	...
	objBallonHutCyan		= _3dsLoadObj(SZ_OBJ_BALLON_HUT_CYAN);
	objBallonHutMagenta		= _3dsLoadObj(SZ_OBJ_BALLON_HUT_MAGENTA);
	objBallonHutBlack		= _3dsLoadObj(SZ_OBJ_BALLON_HUT_BLACK);
	objBallonHutOrange		= _3dsLoadObj(SZ_OBJ_BALLON_HUT_ORANGE);
...
void EngineSaveConfig()
{
	...
	EngineSaveLevel(szTempLevelFfilePath);  //the temp file can always be saved in v3 format, it will guarantee no loss
			...
			if(t->flags & TF_DRAW)
			{
				...
				case T_BUILDING:
					...
					case M_BUILDING_HUT1:
						...
						case OWNER_CYAN:		obj = objHut1Cyan;  goto _3d_hit;
						case OWNER_MAGENTA:		obj = objHut1Magenta;  goto _3d_hit;
						case OWNER_BLACK:		obj = objHut1Black;  goto _3d_hit;
						case OWNER_ORANGE:		obj = objHut1Orange;  goto _3d_hit;
						}
						goto _unknow;
					case M_BUILDING_HUT2:
						switch(t->Thing.Owner)
						...
						case OWNER_CYAN:		obj = objHut2Cyan;  goto _3d_hit;
						case OWNER_MAGENTA:		obj = objHut2Magenta;  goto _3d_hit;
						case OWNER_BLACK:		obj = objHut2Black;  goto _3d_hit;
						case OWNER_ORANGE:		obj = objHut2Orange;  goto _3d_hit;
						}
						goto _unknow;
					case M_BUILDING_HUT3:
						switch(t->Thing.Owner)
						...
						case OWNER_CYAN:		obj = objHut3Cyan;  goto _3d_hit;
						case OWNER_MAGENTA:		obj = objHut3Magenta;  goto _3d_hit;
						case OWNER_BLACK:		obj = objHut3Black;  goto _3d_hit;
						case OWNER_ORANGE:		obj = objHut3Orange;  goto _3d_hit;
						}
						goto _unknow;
					case M_BUILDING_TOWER:
						switch(t->Thing.Owner)
						...
						case OWNER_CYAN:   obj = objTowerCyan;   goto _3d_hit;
						case OWNER_MAGENTA:    obj = objTowerMagenta;    goto _3d_hit;
						case OWNER_BLACK: obj = objTowerBlack; goto _3d_hit;
						case OWNER_ORANGE:  obj = objTowerOrange;  goto _3d_hit;
						}
						goto _unknow;
					case M_BUILDING_SPY_TRAIN:
						switch(t->Thing.Owner)
						...
						case OWNER_CYAN:   obj = objSpyTrainCyan;   goto _3d_hit;
						case OWNER_MAGENTA:    obj = objSpyTrainMagenta;    goto _3d_hit;
						case OWNER_BLACK: obj = objSpyTrainBlack; goto _3d_hit;
						case OWNER_ORANGE:  obj = objSpyTrainOrange;  goto _3d_hit;
						}
						goto _unknow;
					case M_BUILDING_WARRIOR_TRAIN:
						switch(t->Thing.Owner)
						...
						case OWNER_CYAN:   obj = objWarriorTrainCyan;   goto _3d_hit;
						case OWNER_MAGENTA:    obj = objWarriorTrainMagenta;    goto _3d_hit;
						case OWNER_BLACK: obj = objWarriorTrainBlack; goto _3d_hit;
						case OWNER_ORANGE:  obj = objWarriorTrainOrange;  goto _3d_hit;
						}
						goto _unknow;
					case M_BUILDING_TEMPLE:
						switch(t->Thing.Owner)
						...
						case OWNER_CYAN:   obj = objTempleCyan;   goto _3d_hit;
						case OWNER_MAGENTA:    obj = objTempleMagenta;    goto _3d_hit;
						case OWNER_BLACK: obj = objTempleBlack; goto _3d_hit;
						case OWNER_ORANGE:  obj = objTempleOrange;  goto _3d_hit;
						}
						goto _unknow;
					case M_BUILDING_FIREWARRIOR_TRAIN:
						switch(t->Thing.Owner)
						...
						case OWNER_CYAN:   obj = objFwTrainCyan;   goto _3d_hit;
						case OWNER_MAGENTA:    obj = objFwTrainMagenta;    goto _3d_hit;
						case OWNER_BLACK: obj = objFwTrainBlack; goto _3d_hit;
						case OWNER_ORANGE:  obj = objFwTrainOrange;  goto _3d_hit;
						}
						goto _unknow;
					case M_BUILDING_BOAT_HUT_1:
					case M_BUILDING_BOAT_HUT_2:
						...
						case OWNER_CYAN:   obj = objBoatHutCyan;   goto _3d_hit;
						case OWNER_MAGENTA:    obj = objBoatHutMagenta;    goto _3d_hit;
						case OWNER_BLACK: obj = objBoatHutBlack; goto _3d_hit;
						case OWNER_ORANGE:  obj = objBoatHutOrange;  goto _3d_hit;
						}
						goto _unknow;
					case M_BUILDING_AIRSHIP_HUT_1:
					case M_BUILDING_AIRSHIP_HUT_2:
						...
						case OWNER_CYAN:   obj = objBallonHutCyan;   goto _3d_hit;
						case OWNER_MAGENTA:    obj = objBallonHutMagenta;    goto _3d_hit;
						case OWNER_BLACK: obj = objBallonHutBlack; goto _3d_hit;
						case OWNER_ORANGE:  obj = objBallonHutOrange;  goto _3d_hit;
						}
						goto _unknow;
					case M_BUILDING_KNOWLEDGE:
						obj = objKnowledge;
		...
		if (Preset.Land.size() && bLandPresetMode)
		{
			...
			while (az <= Preset.LandSize)
			{
				...
				while (ax <= Preset.LandSize)
				{
					...
					{
						x = (int)ex - (Preset.LandSize / 2);
						z = (int)ez - (Preset.LandSize / 2);
					}
					...
					// Only if height > 0 or user wants to place down water.
					if (h || (Preset.Flags & LPF_PLACE_WATER))
					{
						if (Preset.Flags & LPF_ONLY_HIGH_LAND)
						{
							h2 = wEngineGround[z * GROUND_Z_SIZE + x];
							// Only if the preset land is taller than the original land.
							if (h > h2)
								wEngineGround[z * GROUND_Z_SIZE + x] = h;
						}
						else
						{
							// Normal case
							if (!(Preset.Flags & LPF_ADDITIVE_HEIGHT))
								wEngineGround[z * GROUND_Z_SIZE + x] = h;
							else
							{
								// Additive land height, e.g. place preset land over existing land
								h2 = wEngineGround[z * GROUND_Z_SIZE + x];

								if (h2 + h > GROUND_HEIGHT_MAX)
									h2 = 0;
...
// 0 ~ number (included)
int EngineGetRand(int min, int max)
{
